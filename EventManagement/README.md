...
## Unit Test Projects and Coverage

The solution now includes one focused unit-test project per production project:

- `tests/Ripple.EventManagement.Domain.Tests`
- `tests/Ripple.EventManagement.Application.Tests`
- `tests/Ripple.EventManagement.Infrastructure.Tests`
- `tests/Ripple.EventManagement.Api.Tests`
...
```powershell
dotnet test Ripple.EventManagement.sln --settings coverlet.runsettings --collect:"XPlat Code Coverage"
```

The `coverlet.runsettings` file excludes `Program.cs` from coverage because it is startup composition code and is better covered by smoke/integration tests.
